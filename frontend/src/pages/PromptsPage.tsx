...
const getPromptCompanyLogos = (
  ...
  if (!promptQuestion?.responses || promptQuestion.responses.length === 0)
    return [];
...
const PromptListItem: React.FC<{
  ...
  const companyLogos = promptQuestion
    ? getPromptCompanyLogos(promptQuestion, acceptedCompetitors || [])
    : [];

  // Calculate total citations from all responses (using brands as proxy for citations)
  const totalCitations = promptQuestion
    ? promptQuestion.responses.reduce(
        (total, response) => total + (response.brands?.length || 0),
        0
      )
    : 0;
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm border border-white/20 rounded-lg shadow-md hover:bg-white/85 transition-all mb-3">
      <div className="px-4 py-3">
        <div className="grid grid-cols-[auto_1fr_8rem_6rem_6rem_5rem] gap-3 items-center">
          {/* Number placeholder */}
          <div className="text-xs font-medium text-gray-500">1</div>
...
const PromptsPage: React.FC = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "active" | "inactive" | "suggested"
  >("active");
  const [promptStatusOverrides, setPromptStatusOverrides] = useState<
    Record<string, "active" | "inactive" | "suggested">
  >({});
  ...
  useEffect(() => {
    if (!isEmbedded) {
      setBreadcrumbs([{ label: "AI Performance" }, { label: "Prompts" }]);
    }
  }, [isEmbedded, setBreadcrumbs]);

  // Trials removed: gate prompt editing on subscription status or admin
  useEffect(() => {
    if (!user) {
    ...
    const hasSub = user.subscriptionStatus === "active";
    setCanModifyPrompts(isAdmin || hasSub);
    setIsAdmin(isAdmin);
  }, [user]);
    ...
    if (openQuestionId && promptQuestions.length > 0) {
      ...
      if (question) {
        ...
        const promptItem: PromptItem = {
          ...
          lastUsed:
            question.responses[0]?.createdAt || question.createdAt || "Unknown",
  ...
  const allPrompts = useMemo(() => {
    ...
    const generatedPrompts: PromptItem[] = promptQuestions.map((q) => {
      ...
      if (overrideStatus) {
        ...
        if (q.isActive && q.responses.length === 0) {
          console.warn(
            `Question "${q.question}" is marked active but has no responses - possible processing issue`
          );
        }
      }
    ...
    return showLimit === "all"
      ? filtered
      : filtered.slice(0, parseInt(showLimit, 10));
  }, [
    promptQuestions,
    promptStatusOverrides,
    searchTerm,
    showLimit,
    statusFilter,
  ]);
              ...
              <div className="text-sm text-gray-700">
                {(() => {
                  ...
                  const allActivePrompts = promptQuestions.filter((q) => {
                    const overrideStatus = promptStatusOverrides[q.id];
                    const status =
                      overrideStatus || (q.isActive ? "active" : "suggested");
                    ...
                    <div>
                      ...
                      <div className="px-4 mb-3">
                        <div className="grid grid-cols-[auto_1fr_8rem_6rem_6rem_5rem] gap-3 items-center">
                          ...
                          <div className="text-xs font-medium text-gray-500 uppercase tracking-wide pl-6">
                            Mentions
                          </div>
