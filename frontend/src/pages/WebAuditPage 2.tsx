...
const WebAuditPage: React.FC = () => {
  ...
  const fetchHistory = async () => {
    ...
    try {
      const response = await fetch(`/api/web-audit/companies/${selectedCompany.id}/history`, {
        credentials: 'include',
      });
      if (response.ok) {
  ...
  const startAudit = async () => {
    ...
    try {
      const response = await fetch(`/api/web-audit/companies/${selectedCompany.id}/start`, {
        method: 'POST',
        headers: {
