...
import {
  FANOUT_DISPLAY_LABELS,
  FANOUT_QUESTION_TYPES,
  FanoutQuestionType,
...
const ResponseItem: React.FC<{
  item: FlattenedResponse;
  autoExpand?: boolean;
  ...
  const questionTypeBadge =
    item.questionType && item.questionType in FANOUT_DISPLAY_LABELS
  ...
  const hasIndividualPosition =
    item.position !== null && item.position !== item.bestPosition;

  return (
    <div
      id={`question-${item.questionId}`}
      className="bg-white/80 backdrop-blur-sm rounded-lg shadow-md border border-white/20 overflow-hidden hover:bg-white/85 transition-all duration-200"
    >
        ...
        <div className="grid grid-cols-12 gap-3 items-center">
          ...
          <div className="col-span-7 min-w-0">
            <p
              ...
              title={
                !isExpanded
                  ? `${item.question}\n\nResponse preview: ${stripBrandTags(item.response).substring(0, 200)}${stripBrandTags(item.response).length > 200 ? "..." : ""}`
                  : undefined
              }
            >
              {item.question}
...
const ResponseDetailsPage: React.FC = () => {
    ...
    if (dashboardData?.id && selectedCompany?.id) {
      ...
      const doFetch = async () => {
        try {
          setIsLoading(true);
          const modelParam =
            filters.aiModel === "all" ? undefined : filters.aiModel;
          const typeParam =
            questionTypeFilter === "all" ? undefined : questionTypeFilter;
  ...
  const processedResponses = useMemo(() => {
    ...
    return showLimit === "all"
      ? responses
      : responses.slice(0, parseInt(showLimit, 10));
          ...
          <div className="flex-shrink-0 flex flex-col lg:flex-row lg:justify-between lg:items-center gap-3 mb-2">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:flex items-center gap-2 w-full lg:w-auto">
              ...
              <FilterDropdown
                ...
                onChange={(value) =>
                  setQuestionTypeFilter(value as FanoutQuestionType | "all")
                }
                icon={ListFilter}
                disabled={dashboardLoading || refreshing || isLoading}
              />
