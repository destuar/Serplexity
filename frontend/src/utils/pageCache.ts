...
function getStorage(): Storage | Map<string, string> {
  try {
    ...
    console.warn('[PageCache] SessionStorage unavailable, using memory storage');
  ...
  setItem(key: string, value: string): void {
    if (isMemoryStorage) {
      ...
      try {
        ...
        console.warn('[PageCache] Failed to write to storage, cache disabled:', error);
      }
    }
  },
...
class PageCacheManager {
  ...
  get<T>(key: string): T | null {
    ...
    try {
      ...
      console.error(`[PageCache] Failed to parse cached data for ${key}:`, error);
      delete this.cache[key];
      this.saveCache();
      return null;
    }
  ...
  set<T>(
    ...
    const now = Date.now();
    const expiryMinutes = CACHE_CONFIG.EXPIRY[pageType.toUpperCase() as keyof typeof CACHE_CONFIG.EXPIRY] 
      || CACHE_CONFIG.EXPIRY.DEFAULT;
    ...
    console.log(`[PageCache] Stored: ${key} (expires in ${expiryMinutes}m, size: ${entry.metadata.dataSize} bytes)`);
  }
  ...
  clearCompany(companyId: string): void {
    const keysToDelete = Object.keys(this.cache).filter(key => 
      this.cache[key]?.metadata.companyId === companyId
    );
    ...
    this.saveCache();
    console.log(`[PageCache] Cleared ${keysToDelete.length} entries for company ${companyId}`);
  }
  ...
  clearPageType(pageType: PageType): void {
    const keysToDelete = Object.keys(this.cache).filter(key => 
      this.cache[key]?.metadata.pageType === pageType
    );
    ...
    this.saveCache();
    console.log(`[PageCache] Cleared ${keysToDelete.length} entries for page type ${pageType}`);
  }
  ...
  getStats(): CacheStats {
    ...
    const totalSize = entries.reduce((sum, entry) => sum + entry.metadata.dataSize, 0);
    const timestamps = entries.map(entry => entry.timestamp);
    
    const totalHits = Object.values(this.hitCounts).reduce((sum, hits) => sum + hits, 0);
    const hitRate = this.totalRequests > 0 ? (totalHits / this.totalRequests) * 100 : 0;
  ...
  private checkAndCleanup(): void {
    ...
    if (stats.totalEntries > CACHE_CONFIG.MAX_ENTRIES || 
        stats.totalSize > CACHE_CONFIG.MAX_CACHE_SIZE * CACHE_CONFIG.CLEANUP_THRESHOLD) {
  ...
  private cleanupExpiredEntries(): void {
    ...
    const expiredKeys = Object.keys(this.cache).filter(key => 
      this.cache[key] && now > this.cache[key].expiresAt
    );
    ...
    if (expiredKeys.length > 0) {
      this.saveCache();
      console.log(`[PageCache] Cleaned up ${expiredKeys.length} expired entries`);
    }
  }
...
export function invalidateCache(
  ...
  if (pageType && companyId) {
    ...
    const keys = Object.keys(pageCache['cache']).filter(key => {
      ...
      return entry?.metadata.pageType === pageType && entry?.metadata.companyId === companyId;
    });
...
if (process.env.NODE_ENV === 'development') {
  ...
  (window as any).__SERPLEXITY_CACHE__ = {
    cache: pageCache,
