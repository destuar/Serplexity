 ...
 */
export function usePageCache<T>(options: UseCacheOptions<T>): UseCacheResult<T> {
  const {
    fetcher,
    pageType,
    companyId,
   ...
   */
  const fetchFreshData = useCallback(async (isRefresh = false): Promise<T> => {
    try {
      console.log(`[usePageCache] Fetching ${isRefresh ? 'refresh' : 'fresh'} data for ${pageType}:${companyId}`);
  ...
  const loadData = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      console.error(`[usePageCache] Load failed for ${pageType}:${companyId}:`, err);
  ...
  }, [enabled, companyId, pageType, filters, data, staleWhileRevalidate, fetchFreshData, updateCacheStatus]);
  ...
  const invalidate = useCallback(() => {
    ...
    console.log(`[usePageCache] Invalidated cache for ${pageType}:${companyId}`);
  }, [pageType, companyId]);
...
export function useCachePrefetch() {
  const prefetch = useCallback(<T>(
    ...
    console.log(`[usePageCache] Prefetching data for ${pageType}:${companyId}`);
    
    return fetcher()
      .then(data => {
        setCachedData(pageType, companyId, data, filters);
        console.log(`[usePageCache] Prefetch complete for ${pageType}:${companyId}`);
      })
      .catch(err => {
        console.warn(`[usePageCache] Prefetch failed for ${pageType}:${companyId}:`, err);
      });
  }, []);
