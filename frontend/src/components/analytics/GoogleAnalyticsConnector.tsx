...
const GoogleAnalyticsConnector: React.FC<Props> = ({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (propertyId.trim()) {
        // Manual property ID setup
        await apiClient.post("/website-analytics/integrations", {
          integrationName: "google_analytics_4",
          verificationMethod: "meta_tag",
          ga4PropertyIdOrTag: propertyId.trim(),
        });
        onComplete();
      } else {
      ...
      setError(error.response?.data?.error || error.message || "Failed to create GA4 integration");
