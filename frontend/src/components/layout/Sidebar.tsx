...
const Sidebar: React.FC<SidebarProps> = ({ isCollapsed, toggleSidebar }) => {
  ...
  const getLinkClass = (path: string) => {
    const baseClass = `flex items-center p-3 text-base font-normal rounded-xl transition-all duration-300 ${isCollapsed ? 'justify-center' : ''}`;
  ...
  return (
    <div className={`flex flex-col ${isCollapsed ? 'w-20' : 'w-64'} bg-white/80 backdrop-blur-md border-r border-gray-200/50 text-gray-900 transition-all duration-300`}>
      <div className="flex items-center h-20 border-b border-gray-200/50 px-4">
        <img 
          ...
          className="w-8 h-8"
        />
        {!isCollapsed && <h1 className="text-2xl font-bold ml-2 text-gray-900">Serplexity</h1>}
      </div>
      <div className="flex flex-col p-4">
        ...
        >
          <div className="flex items-center">
            ...
            {!isCollapsed && <span className="ml-2 text-gray-700 font-medium">Market Analysis</span>}
          </div>
          {!isCollapsed && (openSections.marketAnalysis ? <ChevronDown className="text-gray-500" size={16} /> : <ChevronRight className="text-gray-500" size={16} />)}
        </div>
      ...
      <div className="mt-auto p-4">
        ...
        >
          {isCollapsed ? <ChevronsRight size={20} className="text-gray-600" /> : <ChevronsLeft size={20} className="text-gray-600" />}
        </button>
      </div>
    </div>
  );
