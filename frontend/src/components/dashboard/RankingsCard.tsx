...
const RankingsCard = () => {
  ...
  const liveCompetitors = useMemo((): CompetitorData[] => {
    ...
    if (process.env.NODE_ENV === "development") {
      console.log(
        "[RankingsCard] Using live competitor data from context:",
  ...
  const enhancedCompetitors = useMemo((): EnhancedCompetitor[] => {
    ...
    const enhanced = rankingsData.chartCompetitors.map(
      (reportCompetitor): EnhancedCompetitor => {
        ...
        if (liveData) {
          ...
          if (
            ...
            reportCompetitor.website !== liveData.website
          ) {
            console.log(
              `[RankingsCard] Website updated for ${liveData.name}: ${reportCompetitor.website} -> ${liveData.website}`
            );
          }
    ...
    if (process.env.NODE_ENV === "development") {
      const hybridCount = enhanced.filter(
        (c) => c.dataSource === "hybrid"
      ).length;
      const reportOnlyCount = enhanced.filter(
        (c) => c.dataSource === "report"
      ).length;
      console.log(
        `[RankingsCard] Enhanced competitors: ${hybridCount} hybrid, ${reportOnlyCount} report-only`
      );
    }
  ...
  const filteredRankingsData = useMemo(() => {
    ...
    const filteredCompetitors = enhancedCompetitors.filter(
      (competitor) =>
  ...
  const renderIndustryRanking = () => {
    ...
    if (!rankingsData?.industryRanking) {
      const chartData =
        (filteredRankingsData?.chartCompetitors as EnhancedCompetitor[]) || [];
      const displayedChartData = chartData.slice(
        0,
      ...
      return (
        <div className="flex-1 flex flex-col items-center justify-center">
          <div className="text-5xl font-bold text-gray-800 mb-6">N/A</div>
          <div className="flex items-end justify-center space-x-1 h-16 w-full max-w-64">
            {displayedChartData.length > 0
              ? displayedChartData.map(
                  (competitor: EnhancedCompetitor, index: number) => {
                    const maxShareOfVoice = Math.max(
                      ...displayedChartData.map(
                        (c: EnhancedCompetitor) => c.shareOfVoice
                      )
                    );
                    const heightPercent =
                      maxShareOfVoice > 0
                        ? (competitor.shareOfVoice / maxShareOfVoice) * 100
                        : 0;
                    const heightPx = Math.max(
                      8,
                      Math.round((heightPercent / 100) * 64)
                    );
    ...
    const maxShareOfVoice =
      displayedChartData.length > 0
        ? Math.max(
            ...displayedChartData.map((c: EnhancedCompetitor) => c.shareOfVoice)
          )
        : 0;
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        <div className="text-6xl font-bold text-gray-800 mb-6">
          ...
          <span className="text-xl font-normal text-gray-500">
            {getOrdinalSuffix(filteredRankingsData?.industryRanking || 0)}
          </span>
        </div>
        <div className="flex items-end justify-center space-x-1 h-16 w-full max-w-64">
          {displayedChartData.map(
            (competitor: EnhancedCompetitor, index: number) => {
              ...
              return (
                <div
                  ...
                  style={{
                    height: `${heightPx}px`,
                    backgroundColor: isUserCompany
                      ? chartColorArrays.primary[0]
                      : "#d1d5db", // blue-600 : gray-300
                  }}
                  title={`${competitor.name}: ${competitor.shareOfVoice.toFixed(1)}% (${competitor.dataSource})`}
                />
              );
  ...
  const renderMentionsView = () => {
    ...
    if (
      ...
      !filteredRankingsData.chartCompetitors ||
      filteredRankingsData.chartCompetitors.length === 0
    ) {
      return (
        <div className="flex-1 flex items-center justify-center">
    ...
    const remainingShareOfVoice = remainingCompetitors.reduce(
      (total: number, competitor: EnhancedCompetitor) =>
        total + competitor.shareOfVoice,
      0
    );
            ...
            >
              <div className="flex items-center space-x-3 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <span
                    className={`text-sm font-medium block break-words ${
                      isOthers
                      ...
                      {Math.abs(competitor.change ?? 0) < 0.1 ? (
                        ...
                        <div
                          className={`flex items-center text-xs ${
                            competitor.changeType === "increase"
                              ? "text-green-600"
                              : "text-red-600"
                          }`}
                        >
                          ...
                          <span className="ml-1">
                            {Math.abs(competitor.change ?? 0).toFixed(1)}%
                          </span>
                        </div>
                      )}
                    </>
  ...
  const renderCitationsView = () => {
    ...
    if (allSources.length > maxSlots) {
      ...
      const remainingShareOfVoice = remainingSources.reduce(
        (total: number, source: Citation) => total + source.shareOfVoice,
        0
      );
      ...
      displaySources = [
        ...topSources,
        {
          ...
          name: `${remainingCount}+ others`,
          shareOfVoice: remainingShareOfVoice,
          citationCount: remainingSources.reduce(
            (total: number, source: Citation) => total + source.citationCount,
            0
          ),
          uniqueUrls: 0,
        ...
        <div className="w-full lg:w-1/2 pr-0 lg:pr-4 border-r-0 lg:border-r border-b lg:border-b-0 border-gray-200 flex flex-col pb-4 lg:pb-0 mb-4 lg:mb-0">
          <div className="flex items-baseline gap-0.5 mb-4">
            ...
            <Tooltip
              content={
                <span>
                  <strong>Industry Ranking</strong>: your brand's position
                  relative to accepted competitors, based on Share of Voice
                  across the selected models and timeframe. Lower number is
                  better. Use this to gauge competitive standing.
                </span>
              }
            >
              <span
            ...
            <div className="flex items-baseline gap-0.5">
              ...
              <Tooltip
                content={
                  activeTab === "mentions" ? (
                    <span>
                      <strong>Mentions</strong>: top competitors by share of
                      voice in AI answers. Indicates which brands are most
                      frequently referenced alongside yours.
                    </span>
                  ) : (
                    <span>
                      <strong>Citations</strong>: top source domains cited in AI
                      answers. Helps identify authoritative sources to pursue
                      for inclusion.
                    </span>
                  )
                }
