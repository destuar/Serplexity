...
from ..config.models import get_default_model_for_task, ModelTask, ModelEngine
...
class WebsiteEnrichmentAgent(BaseAgent):
    def __init__(self):
        ...
        # Use a proper Perplexity model object when configured, not an OpenAI string alias
        if default_model_config and default_model_config.engine == ModelEngine.PERPLEXITY:
            model_obj = self._create_perplexity_model()
        else:
            # Fallback to configured model id or a sensible default
            model_obj = (
                default_model_config.get_pydantic_model_id()
                if default_model_config
                else os.getenv('PYDANTIC_MODEL_ID', 'openai:gpt-4.1-mini')
            )
...
async def main():
    ...
    try:
        ...
        # Convert result to JSON-serializable format and enforce safe default shape
        if 'result' in result and hasattr(result['result'], 'model_dump'):
            result['result'] = result['result'].model_dump()

        # Ensure we always output a competitors array to satisfy TS client expectations
        if 'result' in result:
            if not isinstance(result['result'], dict) or 'competitors' not in result['result']:
                result['result'] = { 'competitors': [] }
