...
const mockedQueueReport = queueReport as jest.MockedFunction<
  typeof queueReport
>;
...
describe("Report Generation Flow", () => {
  ...
  async function bootstrapUserAndCompany() {
    ...
    const companyRes = await request(app)
      ...
      .send({
        ...
        industry: "Technology",
        competitors: [{ name: "Competitor One", website: "https://comp1.com" }],
        benchmarkingQuestions: [
          "What is the market share of top cloud providers?",
        ],
        products: ["Product X"],
      })
      .expect(201);
  ...
  it("should require authentication when creating a report", async () => {
    await request(app).post(`/api/reports/companies/${companyId}`).expect(401);
  });
  ...
  it("should forbid users without active subscription", async () => {
    ...
    await request(app).post(`/api/reports/companies/${companyId}`).expect(403);
  });
