...
describe('🔗 TypeScript Service Integration Tests', () => {
  ...
  describe('🎯 PydanticLlmService ↔ Agent Integration', () => {
    it('should integrate with all 6 PydanticAI agents via pydanticLlmService', async () => {
      ...
      for (const test of agentTests) {
        try {
          ...
          if (error instanceof Error && error.message.includes('API key')) {
            console.warn(`   ⚠️  ${test.name}: Skipped due to missing API keys`);
            continue;
          }
      ...
      const integrationScore = (successfulAgents.length / agentTests.length) * 100;
  ...
  describe('🔄 LLM Service Integration', () => {
    it('should integrate generateQuestionResponse with question_agent.py', async () => {
      ...
      try {
        ...
        const result = await llmService.generateQuestionResponse(testQuestion, testModel);
        ...
        console.log(`   ✅ Question Response: ${result.data.length} chars, ${result.usage.totalTokens} tokens`);
        console.log(`   📝 Model: ${result.modelUsed}, Usage: ${result.usage.totalTokens} tokens`);
  ...
  describe('📊 Metrics Service Integration', () => {
    it('should validate metrics calculation logic with mock data', async () => {
      ...
      const mockSentimentData = [
        {
          ...
          value: {
            ratings: [
              { quality: 8, priceValue: 7, brandReputation: 9, brandTrust: 8, customerService: 7 }
            ]
          }
        },
        {
          ...
          value: {
            ratings: [
              { quality: 7, priceValue: 8, brandReputation: 8, brandTrust: 7, customerService: 8 }
            ]
          }
        }
      ];
      ...
      const mockFanoutResponses = [
        { response: "TestCorp is a leading technology company...", mentions: { TestCorp: 2, CompetitorA: 1 } },
        { response: "For enterprise solutions, TestCorp offers...", mentions: { TestCorp: 1, CompetitorB: 1 } },
        { response: "When comparing platforms, TestCorp and CompetitorA...", mentions: { TestCorp: 1, CompetitorA: 1 } }
      ];
      ...
      try {
        ...
        const mockMetricsCalculation = {
          ...
          calculateAverageSentiment: (sentiments: any[]) => {
            ...
            const avgQuality = allRatings.reduce((sum, r) => sum + r.quality, 0) / allRatings.length;
            const avgReputation = allRatings.reduce((sum, r) => sum + r.brandReputation, 0) / allRatings.length;
            return { avgQuality, avgReputation };
          },

          // Calculate mention metrics
          calculateMentionMetrics: (responses: any[]) => {
            const totalMentions = responses.reduce((sum, r) => sum + Object.values(r.mentions).reduce((a: any, b: any) => a + b, 0), 0);
            const companyMentions = responses.reduce((sum, r) => sum + (r.mentions.TestCorp || 0), 0);
        ...
        const sentimentMetrics = mockMetricsCalculation.calculateAverageSentiment(mockSentimentData);
        const mentionMetrics = mockMetricsCalculation.calculateMentionMetrics(mockFanoutResponses);
        ...
        console.log(`   ✅ Sentiment Metrics: Quality ${sentimentMetrics.avgQuality.toFixed(1)}, Reputation ${sentimentMetrics.avgReputation.toFixed(1)}`);
        console.log(`   ✅ Mention Metrics: ${mentionMetrics.companyMentions}/${mentionMetrics.totalMentions} mentions (${(mentionMetrics.shareOfVoice * 100).toFixed(1)}% SOV)`);
      ...
      try {
        ...
        const testCases = [
          {
            name: 'Sentiment Analysis',
            script: 'web_search_sentiment_agent.py',
            input: { company_name: "TestCorp", search_queries: ["test"], max_results_per_query: 1 }
          },
          {
            name: 'Question Answering', 
            script: 'question_agent.py',
            input: { company_name: "TestCorp", question: "What is TestCorp?", context: "cost test" }
          }
        ];
        ...
        for (const testCase of testCases) {
          try {
            ...
            console.log(`   💎 ${testCase.name}: ${tokensUsed} tokens, Model: ${result.metadata.modelUsed}`);

          } catch (error) {
            if (error instanceof Error && error.message.includes('API key')) {
              console.warn(`   ⚠️  ${testCase.name}: Skipped due to missing API keys`);
              continue;
            }
    ...
    it('should handle service integration errors gracefully', async () => {
      ...
      try {
        ...
        try {
          ...
          console.log('   ✅ Invalid input handled (may succeed with fallbacks)');
        } catch (error) {
        ...
        const serviceComposition = {
          executeWithFallback: async (primaryAgent: string, fallbackAgent: string, input: any) => {
            try {
              return await pydanticLlmService.executeAgent(primaryAgent, input, null, { timeout: 10000 });
            } catch (primaryError) {
              console.log(`   ⚠️  Primary agent ${primaryAgent} failed, trying fallback...`);
              return await pydanticLlmService.executeAgent(fallbackAgent, input, null, { timeout: 10000 });
            }
          }
        };
        ...
        const fallbackResult = await serviceComposition.executeWithFallback(
          ...
          { company_name: "TestCorp", question: "What is TestCorp?", context: "fallback test" }
        );
