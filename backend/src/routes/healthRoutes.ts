...
router.get('/pydantic', async (req: Request, res: Response) => {
  try {
    ...
    res.status(health.status === 'healthy' ? 200 : health.status === 'degraded' ? 202 : 503).json(health);
...
router.get('/providers', async (req: Request, res: Response) => {
  try {
    ...
    const status = overallHealth > 0.7 ? 'healthy' : overallHealth > 0.3 ? 'degraded' : 'unhealthy';
    ...
    res.status(status === 'healthy' ? 200 : status === 'degraded' ? 202 : 503).json(health);
...
router.get('/overall', async (req: Request, res: Response) => {
  try {
    ...
    const providerHealthScore = providerHealth.filter(p => p.available).length / providerHealth.length;
    ...
    const health = {
      ...
      components: {
        pydanticAI: {
          status: pydanticHealth > 0.7 ? 'healthy' : pydanticHealth > 0.3 ? 'degraded' : 'unhealthy',
          score: pydanticHealth,
          enabled: true,
          activeExecutions: serviceStats.activeExecutions
        },
        providers: {
          status: providerHealthScore > 0.7 ? 'healthy' : providerHealthScore > 0.3 ? 'degraded' : 'unhealthy',
...
router.get('/agents', async (req: Request, res: Response) => {
  try {
    ...
    const health = {
      status: serviceHealth.overallHealth > 0.8 ? 'healthy' : serviceHealth.overallHealth > 0.6 ? 'degraded' : 'unhealthy',
      timestamp: new Date().toISOString(),
      responseTime,
      agents: {
        activeExecutions: serviceStats.activeExecutions,
