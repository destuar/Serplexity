...
if (env.NODE_ENV !== 'test') {
  backupWorker = new Worker('backup-scheduler', async (job: Job) => {
    ...
    if (job.name === 'trigger-backup-daily-reports') {
        ...
        try {
            ...
            console.log(`[Backup Scheduler Worker] Found ${eligibleCompanies.length} companies eligible for daily reports.`);
            ...
            for (const company of eligibleCompanies) {
                ...
                } else if (todaysRun.status === 'PENDING' || todaysRun.status === 'RUNNING') {
                ...
                if (needsBackupReport) {
                    try {
                        console.log(`[Backup Scheduler Worker] Triggering backup report for ${company.name}: ${reason}`);
                        ...
                        console.error(`[Backup Scheduler Worker] Failed to queue backup report for company ${company.id} (${company.name}):`, error);
                        ...
                        await alertingService.alertSystemIssue({
                            ...
                            console.error('[Backup Scheduler Worker] Failed to send backup failure alert:', alertError);
                        });
                    }
                }
            }
            ...
            console.log('[Backup Scheduler Worker] Backup scheduling completed:', {
            ...
            if (backupReportsTriggered === 0) {
                console.log('[Backup Scheduler Worker] ✅ All companies have successful reports today - no backup needed!');
            } else {
                ...
                await alertingService.alertSystemIssue({
                    ...
                    console.error('[Backup Scheduler Worker] Failed to send backup summary alert:', alertError);
                });
            }

        } catch (error) {
            console.error('[Backup Scheduler Worker] Critical error during backup scheduling:', error);
            ...
            await alertingService.alertSystemIssue({
                ...
                console.error('[Backup Scheduler Worker] Failed to send critical error alert:', alertError);
            });
        ...
        console.log('[Backup Scheduler Worker] Processing emergency report trigger...');
        ...
        try {
            ...
            console.log(`[Backup Scheduler Worker] Emergency trigger: processing ${companies.length} companies (Reason: ${reason})`);
            ...
            for (const company of companies) {
                try {
                    ...
                    console.error(`[Backup Scheduler Worker] Emergency trigger failed for company ${company.id} (${company.name}):`, error);
                    failCount++;
                }
            }

            console.log(`[Backup Scheduler Worker] Emergency trigger completed: ${successCount} success, ${failCount} failed`);
            ...
            await alertingService.alertSystemIssue({
                ...
                console.error('[Backup Scheduler Worker] Failed to send emergency completion alert:', alertError);
            });

        } catch (error) {
            console.error('[Backup Scheduler Worker] Critical error during emergency trigger:', error);
            throw error;
        }
    }
}, {
...
backupWorker.on('completed', (job: Job) => { 
    console.log(`[Backup Scheduler Worker] Backup scheduler job ${job.id} (${job.name}) has completed.`);
});

backupWorker.on('failed', async (job: Job | undefined, err: Error) => { 
    if (job) {
        console.error(`[Backup Scheduler Worker] Backup scheduler job ${job.id} (${job.name}) has failed with ${err.message}`);
    } else {
        console.error(`[Backup Scheduler Worker] A backup scheduler job has failed with ${err.message}`);
    }
    ...
    try {
        ...
        console.error('[Backup Scheduler Worker] Failed to send backup scheduler failure alert:', alertError);
    }
});
}
