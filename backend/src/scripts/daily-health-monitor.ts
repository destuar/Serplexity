...
async function performDailyHealthCheck(): Promise<HealthReport> {
  ...
  try {
    ...
    report.recommendations.push(
      "Critical error occurred during health check - manual investigation required",
    );
    throw error;
  }
}
...
async function checkDailyReports(report: HealthReport): Promise<void> {
  ...
  try {
    ...
    const successfulReports = todaysReports.filter(
      (r) => r.status === "COMPLETED",
    ).length;
    const failedReports = todaysReports.filter(
      (r) => r.status === "FAILED",
    ).length;
    const runningReports = todaysReports.filter(
    ...
    const companiesMissingReports = allEligibleCompanies.filter(
      (c) => !companiesWithReports.has(c.id),
    );
    ...
    const missingReportRate =
      (companiesMissingReports.length / eligibleCompanies) * 100;
    ...
    if (companiesMissingReports.length === 0 && successRate >= 95) {
      ...
      report.checks.dailyReports = {
        ...
        details: {
          missingCompanies: companiesMissingReports
            .slice(0, 5)
            .map((c) => c.name),
...
async function checkSchedulerHealth(report: HealthReport): Promise<void> {
  ...
  try {
    ...
    if (now.getHours() >= 6) {
      ...
      if (missingReports === 0) {
      ...
      } else if (missingReports <= expectedReports * 0.1) {
        ...
        report.checks.schedulerHealth = {
          status: "FAIL",
          message:
            "Scheduler appears to have failed - many companies missing reports",
          details: { expectedReports, actualReports, missingReports },
        };
      }
    } else {
...
function generateRecommendations(report: HealthReport): void {
  ...
  if (report.checks.dailyReports.status === "FAIL") {
    recommendations.push(
      "URGENT: Multiple companies missing daily reports - check scheduler and backup scheduler",
    );
    recommendations.push(
      "Consider running emergency report trigger for missing companies",
    );
  } else if (report.checks.dailyReports.status === "WARN") {
    recommendations.push(
      "Monitor companies with missing reports - may need manual report generation",
    );
  }
  ...
  if (report.checks.recentFailures.status === "FAIL") {
    recommendations.push(
      "High failure rate detected - investigate common failure patterns",
    );
  ...
  if (report.checks.systemComponents.status === "FAIL") {
    recommendations.push(
      "URGENT: Critical system components are down - immediate attention required",
    );
  }
  ...
  if (report.checks.schedulerHealth.status === "FAIL") {
    recommendations.push(
      "URGENT: Daily scheduler appears to have failed - check master and backup schedulers",
    );
    recommendations.push(
      "Run manual emergency trigger if scheduler cannot be recovered quickly",
    );
  }

  // Add success rate specific recommendations
  if (
    report.stats.last24Hours.successRate < 90 &&
    report.stats.last24Hours.totalReports > 0
  ) {
    recommendations.push(
      "Success rate below 90% - review error logs and AI model performance",
    );
  }
...
async function displayReport(report: HealthReport): Promise<void> {
  ...
  console.log(
    `   • Companies Missing Reports: ${report.stats.companiesMissingReports}`,
  );
  console.log(
    `   • Reports (24h): ${report.stats.last24Hours.totalReports} total, ${report.stats.last24Hours.successfulReports} successful`,
  );
  console.log(
    `   • Success Rate: ${report.stats.last24Hours.successRate.toFixed(1)}%`,
  );
