...
async function repairFailedReports(dryRun: boolean = true): Promise<void> {
  ...
  console.log(
    "🔧 Analyzing reports marked as FAILED that may have completed successfully...\n",
  );
  ...
  try {
    ...
    console.log(
      `Found ${failedReports.length} failed reports in the last 7 days`,
    );
    ...
    candidates.forEach((candidate, index) => {
      console.log(
        `${index + 1}. ${candidate.companyName} (${candidate.runId})`,
      );
      console.log(`   📅 Created: ${candidate.createdAt.toISOString()}`);
      console.log(`   📊 Has Metrics: ${candidate.hasMetrics ? "✅" : "❌"}`);
      console.log(
        `   📝 Has Responses: ${candidate.hasResponses ? "✅" : "❌"}`,
      );
      console.log(
        `   🎯 Has Tasks: ${candidate.hasOptimizationTasks ? "✅" : "❌"}\n`,
      );
    });
    ...
    if (dryRun) {
      ...
      console.log(
        "Command: npm run script repair-failed-reports -- --repair\n",
      );
      ...
      for (const candidate of candidates) {
        try {
          ...
          console.log(
            `✅ Repaired: ${candidate.companyName} (${candidate.runId})`,
          );
          repairedCount++;
        } catch (error) {
      ...
      if (repairedCount > 0) {
        console.log("\n📝 Next steps:");
        console.log(
          "1. Verify the repaired reports show up correctly in the dashboard",
        );
        console.log(
          "2. Check if any missing post-completion data needs to be regenerated",
        );
        console.log(
          "3. Monitor future reports to ensure the fix prevents this issue",
        );
      }
    }
    ...
    console.log(
      `• Reports with responses: ${withResponses}/${candidates.length}`,
    );
    console.log(
      `• Reports with optimization tasks: ${withTasks}/${candidates.length}`,
    );
    ...
    if (withTasks < candidates.length) {
      ...
      console.log(
        "   Run optimization task generation manually for first reports",
      );
    }
  } catch (error) {
