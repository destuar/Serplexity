    ...
    if (arg === 'monitor') {
      ...
      const monitorInterval = setInterval(async () => {
        try {
          ...
          console.log(`💚 Overall Health: ${(stats.service.overallHealth * 100).toFixed(1)}%\n`);
          
          console.log(`🔗 Providers: ${stats.providers.available}/${stats.providers.total} available`);
          console.log(`🤖 Embedded System Prompts: ${stats.agents.embedSystemPrompts ? 'Yes' : 'No'}`);
          console.log(`⭐ Average Quality: ${(stats.agents.averageQuality * 100).toFixed(1)}%`);
          console.log(`📊 Total Usage: ${stats.agents.totalUsage} requests\n`);
          
          console.log(`⚡ Performance:`);
          console.log(`  Response Time: ${stats.performance.averageResponseTime.toFixed(0)}ms`);
          console.log(`  Success Rate: ${(stats.performance.successRate * 100).toFixed(1)}%`);
          console.log(`  Error Rate: ${(stats.performance.errorRate * 100).toFixed(1)}%`);
      ...
      console.log(`  Active Executions: ${stats.service.activeExecutions}`);
      console.log(`  Pool Size: ${stats.service.poolSize}`);
      console.log(`  Provider Health: ${(stats.providers.available / stats.providers.total * 100).toFixed(1)}%\n`);
      ...
      stats.providers.healthReport.forEach(provider => {
        ...
        console.log(`  ${statusEmoji} ${provider.id}: ${provider.available ? 'Available' : 'Unavailable'} (errors: ${provider.errorCount})`);
      });
      console.log();
      ...
      console.log(`  Success Rate: ${(stats.performance.successRate * 100).toFixed(1)}%`);
      console.log(`  Error Rate: ${(stats.performance.errorRate * 100).toFixed(1)}%`);
      console.log(`  Throughput: ${stats.performance.throughput.toFixed(1)} requests/minute`);
      
      console.log(`\n💡 Usage:`);
      console.log(`  npm run pydantic:stats          - Show current statistics`);
      console.log(`  npm run pydantic:stats monitor  - Start monitoring mode`);
      console.log(`  npm run pydantic:stats export   - Export statistics to JSON`);
    }
