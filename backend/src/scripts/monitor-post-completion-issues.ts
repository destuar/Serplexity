...
async function monitorPostCompletionIssues(): Promise<void> {
  ...
  try {
    ...
    for (const report of suspiciousReports) {
      ...
      if (hasMetrics > 0 || hasResponses > 0) {
        issues.push({
          runId: report.id,
          companyName: report.company.name,
          issue:
            "Report marked as FAILED but has successful data (metrics/responses)",
          severity: "HIGH",
          recommendation:
            "Check post-completion processing logs. Report likely failed during metrics computation, optimization task generation, or archive scheduling.",
    ...
    for (const report of completedReportsWithoutMetrics) {
      issues.push({
        ...
        severity: "MEDIUM",
        recommendation:
          "Run: computeAndPersistMetrics manually for this report. Check database connection and query performance.",
        createdAt: report.createdAt,
      });
    }
    ...
    for (const company of companiesWithFirstReports) {
      ...
      if (firstReport && firstReport.optimizationTasks.length === 0) {
        issues.push({
          ...
          severity: "MEDIUM",
          recommendation:
            "Regenerate optimization tasks manually. Check LLM API connectivity and rate limits.",
    ...
    for (const company of companiesWithManyReports) {
      if (company.runs.length > 5) {
        ...
        if (oldRunIds.length > 0) {
          const oldResponsesCount = await prisma.response.count({
          ...
          if (oldResponsesCount > 0) {
            issues.push({
              ...
              severity: "LOW",
              recommendation:
                "Check archive worker functionality. Old responses may not be getting cleaned up properly.",
              createdAt: new Date(),
            });
          }
        }
    ...
    if (issues.length === 0) {
      ...
      if (highIssues.length > 0) {
        ...
        highIssues.forEach((issue) => {
          console.log(
            `  • ${issue.companyName} (${issue.runId}): ${issue.issue}`,
          );
