...
async function runHealthCheck(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    const healthyProviders = providerHealth.filter(p => p.available && p.errorCount < 5);
    const degradedProviders = providerHealth.filter(p => p.available && p.errorCount >= 5);
    ...
    const overallScore = (providerScore * 0.5 + serviceScore * 0.3 + promptScore * 0.2);
    ...
    if (unhealthyProviders.length > 0) {
      recommendations.push(`${unhealthyProviders.length} provider(s) are unhealthy. Check API keys and connectivity.`);
    }
    
    if (degradedProviders.length > 0) {
      recommendations.push(`${degradedProviders.length} provider(s) have high error rates. Monitor for issues.`);
    }
    
    if (serviceStats.activeExecutions > 20) {
      recommendations.push('High number of active executions. Consider scaling up or optimizing performance.');
    }
    
    if (avgPromptPerformance < 0.7) {
      recommendations.push('Low agent performance detected. Review and optimize agent system prompts.');
    }
    ...
    const result: HealthCheckResult = {
      ...
      providers: {
        ...
        details: providerHealth.map(p => ({
          ...
          status: p.available ? (p.errorCount < 5 ? 'healthy' : 'degraded') : 'unhealthy'
        }))
      },
      service: {
        activeExecutions: serviceStats.activeExecutions,
        poolSize: serviceStats.poolSize,
        status: serviceHealth.overallHealth > 0.7 ? 'healthy' : serviceHealth.overallHealth > 0.3 ? 'degraded' : 'unhealthy'
      },
      agents: {
        embedSystemPrompts: true,
        averagePerformance: avgPromptPerformance,
        status: avgPromptPerformance > 0.8 ? 'healthy' : avgPromptPerformance > 0.6 ? 'degraded' : 'unhealthy'
      },
      recommendations
    };
...
async function main() {
  try {
    ...
    // Display results
    console.log(`📊 Overall Health: ${healthResult.overall.status.toUpperCase()}`);
    console.log(`🎯 Health Score: ${(healthResult.overall.score * 100).toFixed(1)}%`);
    ...
    if (healthResult.providers.details.length > 0) {
      ...
      healthResult.providers.details.forEach(provider => {
        const statusEmoji = provider.status === 'healthy' ? '✅' : provider.status === 'degraded' ? '⚠️' : '❌';
        console.log(`  ${statusEmoji} ${provider.id}: ${provider.status} (errors: ${provider.errorCount})`);
      });
      console.log();
    }
    
    console.log('⚙️ Service Status:');
    console.log(`  Active Executions: ${healthResult.service.activeExecutions}`);
    console.log(`  Pool Size: ${healthResult.service.poolSize}`);
    console.log(`  Status: ${healthResult.service.status}\n`);
    
    console.log('🤖 Agent Performance:');
    console.log(`  Embedded System Prompts: ${healthResult.agents.embedSystemPrompts ? 'Yes' : 'No'}`);
    console.log(`  Average Performance: ${(healthResult.agents.averagePerformance * 100).toFixed(1)}%`);
