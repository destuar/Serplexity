...
export interface CreateIntegrationRequest {
  companyId: string;
  integrationName: 'google_search_console' | 'manual_tracking';
...
class WebsiteAnalyticsService {
    ...
    try {
      ...
      if (request.integrationName === 'google_search_console') {
        if (request.verificationMethod === 'oauth') {
          // OAuth flow - integration will be completed after OAuth callback
          integrationData.gscPropertyUrl = request.gscPropertyUrl;
        } else {
        ...
        const verificationToken = analyticsVerificationService.generateVerificationToken(company.website);
        const trackingCode = this.generateWebsiteTrackingCode(request.companyId, company.website);
      ...
      logger.info(`Created website analytics integration ${integration.id} for company ${request.companyId}`);
  ...
  async completeOAuthIntegration(
    ...
    try {
      ...
      const tokens = await googleSearchConsoleService.getTokensFromCode(authCode);
  ...
  async verifyManualIntegration(integrationId: string): Promise<any> {
    try {
      const result = await analyticsVerificationService.verifyIntegration(integrationId);
   ...
   */
  async getCompanyIntegrations(companyId: string): Promise<IntegrationStatus[]> {
    try {
      const prisma = await dbCache.getPrimaryClient();
      ...
      return integrations.map(integration => {
        ...
        } else if (integration.integrationName === 'google_search_console' && integration.lastSyncAt) {
   ...
   */
  private generateWebsiteTrackingCode(companyId: string, websiteUrl: string): string {
...
<!-- End Serplexity Website Analytics -->
  ...
  private calculateWebsiteMetrics(data: any[]): WebsiteAnalyticsMetrics {
    ...
    const weightedPositionSum = data.reduce((sum, row) => 
    ...
    data.forEach(row => {
      if (row.query) {
        const existing = queryStats.get(row.query) || { 
          query: row.query, impressions: 0, clicks: 0, totalPosition: 0, count: 0 
        };
    ...
    data.forEach(row => {
      if (row.page) {
        const existing = pageStats.get(row.page) || { 
          page: row.page, impressions: 0, clicks: 0, totalPosition: 0, count: 0 
        };
        existing.impressions += row.impressions || 0;
        existing.clicks += row.clicks || 0;
        existing.totalPosition += (row.position || 0) * (row.impressions || 0);
    ...
    data.forEach(row => {
      if (row.deviceType) {
        const existing = deviceStats.get(row.deviceType) || { 
          device: row.deviceType, impressions: 0, clicks: 0 
        };
        existing.impressions += row.impressions || 0;
        existing.clicks += row.clicks || 0;
    ...
    const deviceBreakdown = Array.from(deviceStats.values())
      .map(stat => ({
        device: stat.device,
        impressions: stat.impressions,
        clicks: stat.clicks,
    ...
    data.forEach(row => {
      if (row.country) {
        const existing = countryStats.get(row.country) || { 
          country: row.country, impressions: 0, clicks: 0 
        };
        existing.impressions += row.impressions || 0;
        existing.clicks += row.clicks || 0;
    ...
    data.forEach(row => {
      ...
      const existing = dateStats.get(dateKey) || { 
        date: dateKey, impressions: 0, clicks: 0, totalPosition: 0, count: 0 
      };
      existing.impressions += row.impressions || 0;
      existing.clicks += row.clicks || 0;
      existing.totalPosition += (row.position || 0) * (row.impressions || 0);
