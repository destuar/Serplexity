...
class SecurityAnalyzer {
  ...
  private async analyzeHTTPS(
    ...
    try {
      ...
      const certificateValid = await this.validateSSLCertificate(
        parsedUrl.hostname,
        parsedUrl.port || "443"
      );
  ...
  private async analyzeSecurityHeaders(
    url: string
  ): Promise<SecurityResults["headers"]> {
    try {
      ...
      return {
        contentSecurityPolicy: this.hasSecurityHeader(
          headers,
          "content-security-policy"
        ),
        xFrameOptions: this.hasSecurityHeader(headers, "x-frame-options"),
        xContentTypeOptions: this.hasSecurityHeader(
          headers,
          "x-content-type-options"
        ),
        referrerPolicy: this.hasSecurityHeader(headers, "referrer-policy"),
        permissionsPolicy:
          this.hasSecurityHeader(headers, "permissions-policy") ||
  ...
  private hasSecurityHeader(
    ...
    switch (headerName.toLowerCase()) {
      ...
      case "x-frame-options":
        return (
          ["deny", "sameorigin"].includes(value) ||
  ...
  private async detectVulnerabilities(
    url: string,
    headers: SecurityResults["headers"]
    ...
    try {
      ...
      if (!headers.contentSecurityPolicy) {
        vulnerabilities.push({
          ...
          severity: "high",
          description:
            "Content Security Policy (CSP) header is missing, making the site vulnerable to XSS attacks",
        });
      }
      ...
      if (!headers.xFrameOptions) {
        vulnerabilities.push({
          ...
          severity: "medium",
          description:
            "X-Frame-Options header is missing, site may be vulnerable to clickjacking attacks",
        });
      }
      ...
      if (!headers.xContentTypeOptions) {
        vulnerabilities.push({
          ...
          severity: "medium",
          description:
            "X-Content-Type-Options header is missing, site may be vulnerable to MIME-type confusion attacks",
        });
      }

      // Server information disclosure
      const serverHeader = responseHeaders.server as unknown;
      if (serverHeader && serverHeader.toString().includes("/")) {
        vulnerabilities.push({
          type: "server-version-disclosure",
          severity: "low",
          description:
            "Server header reveals version information that could aid attackers",
        });
      }
      ...
      if (parsedUrl.protocol === "https:") {
        ...
        const tlsVersion = (responseHeaders["tls-version"] ||
          responseHeaders["ssl-version"]) as string | undefined;
        if (
          typeof tlsVersion === "string" &&
          (tlsVersion.includes("1.0") || tlsVersion.includes("1.1"))
        ) {
          vulnerabilities.push({
            type: "weak-tls",
            severity: "high",
            description:
              "Site uses outdated TLS version (1.0 or 1.1), which is vulnerable to attacks",
          });
        }
      }
        ...
        for (const cookie of cookies) {
          ...
          if (!cookieStr.includes("httponly")) {
            vulnerabilities.push({
              ...
              severity: "medium",
              description:
                "Cookies are not marked as HttpOnly, making them vulnerable to XSS",
            });
            break;
          }
        }
