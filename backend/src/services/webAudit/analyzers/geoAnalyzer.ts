...
class GEOAnalyzer {
   ...
   */
  private analyzeSchemaMarkup($: cheerio.CheerioAPI): GEOResults['schemaMarkup'] {
    ...
    $('script[type="application/ld+json"]').each((_, element) => {
      try {
        ...
        if (jsonData['@type'] || (Array.isArray(jsonData) && jsonData[0]?.['@type'])) {
   ...
   */
  private analyzeContentStructure($: cheerio.CheerioAPI): GEOResults['contentStructure'] {
   ...
   * Analyze AI optimization factors
   */
  private analyzeAIOptimization($: cheerio.CheerioAPI): GEOResults['aiOptimization'] {
  ...
  private detectFAQSections($: cheerio.CheerioAPI): number {
    ...
    $('script[type="application/ld+json"]').each((_, element) => {
      try {
        ...
        if (jsonData['@type'] === 'FAQPage' || 
            (Array.isArray(jsonData) && jsonData.some((item: any) => item['@type'] === 'FAQPage'))) {
          faqCount++;
        }
      } catch (error) {
    ...
    const questionElements = $('h1, h2, h3, h4, h5, h6, dt, .question, [class*="question"]');
    let questionAnswerPairs = 0;

    questionElements.each((_, element) => {
      const text = $(element).text().toLowerCase();
      if (text.includes('?') || text.match(/^(what|how|why|when|where|who|which)/)) {
        questionAnswerPairs++;
      }
    });
  ...
  private detectAnswerReadyContent($: cheerio.CheerioAPI): number {
    ...
    const highlightElements = $('.highlight, .callout, .note, .tip, .important, blockquote');
  ...
  private calculateReadabilityScore($: cheerio.CheerioAPI): number {
    ...
    const sentences = bodyText.split(/[.!?]+/).filter(sentence => sentence.trim().length > 0);
    ...
    const readabilityIndex = (0.39 * avgWordsPerSentence) + (11.8 * avgCharsPerWord) - 15.59;
  ...
  private detectCitationFriendlyContent($: cheerio.CheerioAPI): boolean {
    ...
    const authorElements = $('[rel="author"], .author, .byline, [class*="author"]');
    if (authorElements.length > 0) {
      citationScore += 20;
    }

    // Check for publication date
    const dateElements = $('time, .date, .published, [datetime], [class*="date"]');
    ...
    const referenceElements = $('.references, .sources, .citations, [class*="reference"]');
    ...
    $('script[type="application/ld+json"]').each((_, element) => {
      try {
        ...
        if (jsonData['@type'] === 'Question' || 
            (Array.isArray(jsonData) && jsonData.some((item: any) => item['@type'] === 'Question'))) {
          structuredCount++;
        }
      } catch (error) {
        // Ignore parsing errors
