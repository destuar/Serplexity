...
class AuditScorer {
  ...
  private calculatePerformanceScore(performance?: PerformanceResults): number {
    ...
    const resourceScore = this.scoreResourceOptimization(
  ...
  private calculateGEOScore(geo?: GEOResults): number {
    ...
    // Schema markup presence & validity (15)
    if (geo.schemaMarkup.totalSchemas > 0) {
      const validSchemas = geo.schemaMarkup.jsonLd.filter(
        (s) => s.valid
      ).length;
      const schemaQuality =
        validSchemas / Math.max(1, geo.schemaMarkup.totalSchemas);
      score +=
        15 *
        Math.min(1, schemaQuality + (geo.schemaMarkup.microdata ? 0.1 : 0));
    }

    // Schema completeness (15)
    if (geo.aiOptimization.schemaCompletenessScore !== undefined) {
      score += Math.min(
        15,
        (geo.aiOptimization.schemaCompletenessScore / 100) * 15
      );
    }

    // Content structure & answer-ready (15)
    const structureSub =
      Math.min(6, Math.min(10, geo.contentStructure.faqSections * 2) * 0.6) +
      Math.min(5, Math.min(8, geo.contentStructure.listStructure * 1) * 0.625) +
      Math.min(
        4,
        Math.min(7, geo.contentStructure.answerReadyContent * 0.5) * 0.5714
      );
    score += Math.min(15, structureSub);

    // Readability (10)
    score += Math.min(10, (geo.aiOptimization.readabilityScore / 100) * 10);

    // Chunkability (10)
    if (geo.aiOptimization.chunkabilityScore !== undefined) {
      score += Math.min(10, (geo.aiOptimization.chunkabilityScore / 100) * 10);
    }

    // Anchor coverage (5)
    if (geo.aiOptimization.anchorCoverage !== undefined) {
      score += Math.min(5, (geo.aiOptimization.anchorCoverage / 100) * 5);
    }

    // Question heading coverage (5)
    if (geo.aiOptimization.questionHeadingCoverage !== undefined) {
      score += Math.min(
        5,
        (geo.aiOptimization.questionHeadingCoverage / 100) * 5
      );
    }

    // Freshness (10)
    if (geo.aiOptimization.freshnessScore !== undefined) {
      score += Math.min(10, (geo.aiOptimization.freshnessScore / 100) * 10);
    }

    // TL;DR present (2)
    if (geo.aiOptimization.tldrPresent) {
      score += 2;
    }

    // Citation friendly (3) + Structured answers (5)
    if (geo.aiOptimization.citationFriendly) score += 3;
  ...
  generateRecommendations(
    ...
    try {
      ...
      if (details.seo) {
        recommendations.push(
          ...this.generateSEORecommendations(details.seo, scores.seo)
        );
      }
      ...
      if (details.geo) {
        recommendations.push(
          ...this.generateGEORecommendations(details.geo, scores.geo)
        );
      }
      ...
      logger.info("Generated recommendations", {
        ...
        byCategory: {
          performance: recommendations.filter(
  ...
  private scoreResourceOptimization(
    pageSize: number,
  ...
  private generatePerformanceRecommendations(
    ...
    if (performance.coreWebVitals.lcp > 2500) {
      recommendations.push({
        ...
        description: `Your LCP is ${(performance.coreWebVitals.lcp / 1000).toFixed(2)} seconds. Optimize main content loading to under 2.5 seconds.`,
  ...
  private generateSEORecommendations(
    ...
    if (!seo.technical.llmsTxt.exists) {
      recommendations.push({
        ...
        description:
          "Add llms.txt to control AI training data usage of your content",
  ...
  private generateGEORecommendations(
    ...
    if (geo.schemaMarkup.totalSchemas === 0) {
      recommendations.push({
        ...
        description:
          "Implement structured data (JSON-LD) to help AI understand your content",
    ...
    if (geo.aiOptimization.readabilityScore < 70) {
      recommendations.push({
        ...
        description:
          "Simplify sentence structure and add more headings and lists",
