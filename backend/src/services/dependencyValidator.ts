...
export class DependencyValidator {
    ...
    this.checks = [
      {
        ...
        remediation:
          "Install Python 3.8+ or set PYTHON_PATH environment variable",
      },
      {
      ...
      },
      {
        ...
        remediation:
          "Run: pip3 install -r requirements.txt --upgrade (Note: pydantic-ai+pydantic-ai-slim conflicts are expected and safe)",
      },
      {
        ...
        remediation:
          "Check database connection string and network connectivity",
      },
      {
  ...
  public async validateAll(failFast: boolean = false): Promise<{
    ...
    logger.info(
      "[DependencyValidator] Starting comprehensive dependency validation"
    );
      ...
      try {
        ...
        if (!result.success) {
          if (check.required) {
            ...
            if (failFast) {
              logger.error(
                `[DependencyValidator] Critical dependency failed: ${check.name}`,
                {
          ...
          logger.info(
            `[DependencyValidator] ✅ ${check.name} validated successfully`
          );
        }
      } catch (error) {
        const errorMessage =
    ...
    if (overallSuccess) {
      logger.info(
        "[DependencyValidator] ✅ All critical dependencies validated successfully",
        {
      ...
      logger.error(
        "[DependencyValidator] ❌ Critical dependency validation failed",
        {
  ...
  private async validatePydanticAI(): Promise<DependencyResult> {
    ...
    return new Promise((resolve) => {
      const proc = spawn(pythonPath, [
        "-c",
        "import pydantic_ai; print(pydantic_ai.__version__)",
      ]);
      let output = "";
      let errorOutput = "";
  ...
  private async validateRequirementsSync(): Promise<DependencyResult> {
    try {
        ...
        proc.on("close", (code) => {
          if (code === 0) {
              ...
              for (const [expectedName, expectedVersion] of expectedPackages) {
                ...
                const installed = installedPackages.find(
                  (pkg: { name: string; version: string }) => {
                    const pkgNameNormalized = pkg.name
                      .toLowerCase()
                      .replace(/_/g, "-");
                    const expectedNameNormalized = expectedName.replace(
                      /_/g,
                      "-"
                    );
                    return (
                      pkgNameNormalized === expectedNameNormalized ||
                ...
                if (!installed) {
                  ...
                  !installed.version.includes(expectedVersion.split("==")[0])
                ) {
                  mismatches.push(
            ...
            resolve({
              ...
              remediation:
                "Check pip installation and run: pip3 install -r requirements.txt",
            });
          }
        });
      });
      ...
      return {
        ...
        remediation:
          "Ensure requirements.txt exists and run: pip3 install -r requirements.txt",
      };
    }
  }
    ...
    try {
      ...
      if (missingFiles.length === 0 && missingDirs.length === 0) {
        ...
        return {
          ...
          remediation:
            "Ensure all required Python agent files and directories exist",
        };
      }
    } catch (error) {
      return {
        ...
        remediation:
          "Ensure pydantic_agents directory exists with required Python files",
      };
    }
  }
