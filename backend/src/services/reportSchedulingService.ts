...
const schedulingLog = (context: SchedulingLogContext, message: string, level: 'INFO' | 'WARN' | 'ERROR' | 'DEBUG' = 'INFO') => {
...
export async function queueReport(companyId: string, force = false): Promise<QueueResult> {
    ...
    try {
        ...
        if (!force) {
            ...
            schedulingLog({ companyId, force, step: 'EXISTING_CHECK' }, 'Checking for existing report runs today');
            ...
            schedulingLog({ companyId, force, step: 'FORCE_SKIP_CHECK' }, 'Force mode enabled - skipping existing run check');
        }
        
        // Fetch company data
        const companyFetchTimer = Date.now();
        schedulingLog({ companyId, force, step: 'COMPANY_FETCH' }, 'Fetching company data and competitors');
        ...
        if (!company) {
            ...
            }, `Company not found: ${companyId}`, 'ERROR');
        ...
        schedulingLog({ companyId, force, step: 'REPORT_CREATE' }, 'Creating new report run in database');
